...
function Producer(queueName, options){
    ...
    this._taskTTL = this._options.taskTTL || 3000; // TODO: it doesn't use
...
Producer.prototype.add = function (data, callback) {
    var id = uuid();
    var message = {data, id, ttl: Date.now() + this._taskTTL};
    var isAck = false;
...
Producer.prototype.getTasks = function (callback) {
    this._client.lrange(this._queueName, 0, -1, callback);
};
